...
## the reprobuild test graph, the Justfile recipes (which also pass their own
## `--nimcache:` under `build/`) and a bare `nim c` alike, whatever the working directory.
...
## Nothing in this repository passes `--skipParentCfg` today.  Its tests run through
