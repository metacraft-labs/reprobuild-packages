## Smoke test for the from-source ``qt6Wayland`` recipe (M9.R.15p.1.1).
##
...
# fetch spec + library artifacts under ``qt6Wayland`` at
# module init time.
import ./repro
...
suite "qt6Wayland — from-source recipe smoke test":

  test "fetch spec carries the upstream URL verbatim":
    let spec = registeredFetchSpec("qt6Wayland")
    check spec.packageName == "qt6Wayland"
  ...
  test "artifacts register the three Qt6 Wayland libraries":
    let arts = registeredArtifacts("qt6Wayland")
    check arts.len == 3
    check arts[0].packageName == "qt6Wayland"
  ...
  test "versions block records the upstream tag + URL + repository":
    let vs = registeredVersions("qt6Wayland")
    check vs.len == 1
