...
suite "libsoup3 source recipe":
  test "pins the verified GNOME release":
    let spec = registeredFetchSpec("libsoup3")
  ...
  test "registers the HTTP library and mandatory closure":
    check registeredArtifacts("libsoup3")[0].artifactName == "libSoup3"
    let deps = registeredBuildDeps("libsoup3")
