## Smoke test for the from-source ``kitemmodels`` recipe
## (M9.R.15q.1.7).
...
suite "kitemmodels — from-source recipe smoke test":

  test "fetch spec carries the upstream URL verbatim":
    let spec = registeredFetchSpec("kitemmodels")
    check spec.packageName == "kitemmodels"
  ...
  test "artifacts register libKF6ItemModels":
    let arts = registeredArtifacts("kitemmodels")
    check arts.len == 1
    check arts[0].packageName == "kitemmodels"
    check arts[0].artifactName == "libKF6ItemModels"
    check arts[0].kind == dakLibrary

  test "versions block records the upstream tag + URL + repository":
    let vs = registeredVersions("kitemmodels")
    check vs.len == 1
