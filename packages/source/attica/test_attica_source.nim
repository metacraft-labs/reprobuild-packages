## Smoke test for the from-source ``attica`` recipe (M9.R.15q.10.1).
...
suite "attica — from-source recipe smoke test":

  test "fetch spec carries the upstream URL verbatim":
    let spec = registeredFetchSpec("attica")
    check spec.packageName == "attica"
  ...
  test "artifacts register libKF6Attica":
    let arts = registeredArtifacts("attica")
    check arts.len == 1
    check arts[0].packageName == "attica"
    check arts[0].artifactName == "libKF6Attica"
    check arts[0].kind == dakLibrary

  test "versions block records the upstream tag + URL + repo":
    let vs = registeredVersions("attica")
    check vs.len == 1
