## Smoke test for the from-source ``libvorbis`` recipe (M9.R.15p.2.3).
...
suite "libvorbis — from-source recipe smoke test":

  test "fetch spec carries the upstream URL verbatim":
    let spec = registeredFetchSpec("libvorbis")
  ...
  test "artifacts register the three libvorbis library outputs":
    let arts = registeredArtifacts("libvorbis")
    check arts.len == 3
    let names = block:
      var s: seq[string]
  ...
  test "versions block records the upstream tag + URL + repository":
    let vs = registeredVersions("libvorbis")
    check vs.len == 1
