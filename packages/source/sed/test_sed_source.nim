## Smoke test for the from-source ``sed`` recipe.
##
...
suite "sed — from-source recipe smoke test":

  test "fetch spec carries the upstream URL verbatim":
    # M9.H registry round-trip — URL is recorded exactly as declared.
    let spec = registeredFetchSpec("sed")
    check spec.packageName == "sed"
  ...
  test "artifacts register a single sed executable tagged dakExecutable":
    ...
    let arts = registeredArtifacts("sed")
    check arts.len == 1
    check arts[0].packageName == "sed"
