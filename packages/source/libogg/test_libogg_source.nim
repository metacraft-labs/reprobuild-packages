## Smoke test for the from-source ``libogg`` recipe (M9.R.15p.2.2).
...
suite "libogg — from-source recipe smoke test":

  test "fetch spec carries the upstream URL verbatim":
    let spec = registeredFetchSpec("libogg")
  ...
  test "artifacts register libOgg":
    let arts = registeredArtifacts("libogg")
    check arts.len == 1
    check arts[0].artifactName == "libOgg"
    check arts[0].kind == dakLibrary

  test "versions block records the upstream tag + URL + repository":
    let vs = registeredVersions("libogg")
    check vs.len == 1
