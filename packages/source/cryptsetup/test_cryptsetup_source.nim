## Smoke test for the from-source ``cryptsetup`` recipe — closes
## M9.R.27 Gap 4.
...
suite "cryptsetup — from-source recipe smoke test":

  test "fetch spec carries the upstream URL verbatim":
    let spec = registeredFetchSpec("cryptsetup")
    check spec.packageName == "cryptsetup"
  ...
  test "build dependencies cover source runtime libraries":
    check registeredNativeBuildDeps("cryptsetup") == @[
      "autoconf", "automake", "libtool", "m4", "make", "gcc >=11",
      "pkg-config", "gettext",
    ]
    check registeredBuildDeps("cryptsetup") == @[
      "libgcrypt", "json-c", "popt", "lvm2", "util-linux",
    ]

  test "tools and shared library are registered":
    let artifacts = registeredArtifacts("cryptsetup")
