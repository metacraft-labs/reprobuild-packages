...
package gccSource:
  ...
  executable gcc:
    ...
    build:
      ...
      shell "LD_LIBRARY_PATH=; export LD_LIBRARY_PATH; bootstrap_sysroot=$extracted/bootstrap-sysroot; glibc_include=$(clang -E -Wp,-v -xc /dev/null 2>&1 | grep glibc | tail -n1); glibc_include=${glibc_include# }; glibc_lib=$(dirname $(clang --print-file-name=libc.so.6)); test -d $glibc_include; test -f $glibc_lib/libc.so.6; mkdir -p $extracted/build $bootstrap_sysroot/usr; ln -sfn $glibc_include $bootstrap_sysroot/usr/include; ln -sfn $glibc_lib $bootstrap_sysroot/lib; ln -sfn $glibc_lib $bootstrap_sysroot/lib64; ln -sfn $glibc_lib $bootstrap_sysroot/usr/lib"
      ...
      # files in a complete sysroot. This breaks the GCC/glibc bootstrap cycle
      # without letting libstdc++'s include_next directives fall back to host
      # headers.
      shell "mkdir -p $out/lib/bootstrap-sysroot/usr $out/include; for runtime in libgcc_s.so libgcc_s.so.1 libstdc++.so libstdc++.so.6 libgomp.so libgomp.so.1 libatomic.so libatomic.so.1; do test -e $out/lib64/$runtime; ln -sfn ../lib64/$runtime $out/lib/$runtime; done; bootstrap_lib=$(readlink -f $extracted/bootstrap-sysroot/lib); bootstrap_include=$(readlink -f $extracted/bootstrap-sysroot/usr/include); test -d $bootstrap_lib; test -d $bootstrap_include; ln -sfn $bootstrap_include $out/include/bootstrap-libc; ln -sfn ../../../include/bootstrap-libc $out/lib/bootstrap-sysroot/usr/include; ln -sfn .. $out/lib/bootstrap-sysroot/lib; ln -sfn .. $out/lib/bootstrap-sysroot/lib64; ln -sfn ../.. $out/lib/bootstrap-sysroot/usr/lib; for runtime in $bootstrap_lib/*.o $bootstrap_lib/*.a $bootstrap_lib/*.so $bootstrap_lib/*.so.*; do test -e $runtime || continue; name=$(basename $runtime); test -e $out/lib/$name || ln -s $runtime $out/lib/$name; done; test -e $out/lib/ld-linux-x86-64.so.2; wrapper_src=$(dirname $extracted)/gcc-driver-wrapper.c; test -f $wrapper_src; for driver in gcc g++; do mv $out/bin/$driver $out/bin/$driver.real; $out/bin/gcc.real --sysroot=$out/lib/bootstrap-sysroot -O2 -Wl,--dynamic-linker=$bootstrap_lib/ld-linux-x86-64.so.2 -Wl,-rpath,$bootstrap_lib -DREAL_DRIVER=\\\"$driver.real\\\" $wrapper_src -o $out/bin/$driver; done; rm -f $out/bin/c++; ln -s g++ $out/bin/c++"
