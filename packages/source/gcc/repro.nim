...
package gccSource:
  ...
  executable gcc:
    ...
    build:
      ...
      # bootstrap sysroot.
      shell "mkdir -p $out/lib $out/include; for runtime in libgcc_s.so libgcc_s.so.1 libstdc++.so libstdc++.so.6 libgomp.so libgomp.so.1 libatomic.so libatomic.so.1; do test -e $out/lib64/$runtime; ln -sfn ../lib64/$runtime $out/lib/$runtime; done; bootstrap_lib=$(readlink -f $extracted/bootstrap-sysroot/lib); bootstrap_include=$(readlink -f $extracted/bootstrap-sysroot/usr/include); test -d $bootstrap_lib; test -d $bootstrap_include; ln -sfn $bootstrap_include $out/include/bootstrap-libc; for runtime in $bootstrap_lib/*.o $bootstrap_lib/*.a $bootstrap_lib/*.so $bootstrap_lib/*.so.*; do test -e $runtime || continue; name=$(basename $runtime); test -e $out/lib/$name || ln -s $runtime $out/lib/$name; done; for driver in gcc g++; do mv $out/bin/$driver $out/bin/$driver.real; printf '#!/bin/sh\nprefix=$(CDPATH= cd \"$(dirname \"$0\")/..\" && pwd)\nexec \"$prefix/bin/%s.real\" -isystem \"$prefix/include/bootstrap-libc\" -Wl,--dynamic-linker=%s/ld-linux-x86-64.so.2 \"$@\"\n' \"$driver\" \"$bootstrap_lib\" > $out/bin/$driver; chmod +x $out/bin/$driver; done"
