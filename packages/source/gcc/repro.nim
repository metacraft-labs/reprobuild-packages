...
package gccSource:
  ...
  executable gcc:
    ...
    build:
      ...
      # headers.
      shell "mkdir -p $out/lib $out/include $out/bootstrap-sysroot/usr; for runtime in libgcc_s.so libgcc_s.so.1 libstdc++.so libstdc++.so.6 libgomp.so libgomp.so.1 libatomic.so libatomic.so.1; do test -e $out/lib64/$runtime; ln -sfn ../lib64/$runtime $out/lib/$runtime; done; bootstrap_lib=$(readlink -f $extracted/bootstrap-sysroot/lib); bootstrap_include=$(readlink -f $extracted/bootstrap-sysroot/usr/include); test -d $bootstrap_lib; test -d $bootstrap_include; ln -sfn $bootstrap_include $out/include/bootstrap-libc; ln -sfn ../../include/bootstrap-libc $out/bootstrap-sysroot/usr/include; ln -sfn ../lib $out/bootstrap-sysroot/lib; ln -sfn ../lib $out/bootstrap-sysroot/lib64; ln -sfn ../../lib $out/bootstrap-sysroot/usr/lib; for runtime in $bootstrap_lib/*.o $bootstrap_lib/*.a $bootstrap_lib/*.so $bootstrap_lib/*.so.*; do test -e $runtime || continue; name=$(basename $runtime); test -e $out/lib/$name || ln -s $runtime $out/lib/$name; done; test -e $out/lib/ld-linux-x86-64.so.2; for driver in gcc g++; do mv $out/bin/$driver $out/bin/$driver.real; printf '#!/bin/sh\nprefix=$(CDPATH= cd \"$(dirname \"$0\")/..\" && pwd)\nexec \"$prefix/bin/%s.real\" --sysroot=\"$prefix/bootstrap-sysroot\" -Wl,--dynamic-linker=\"$prefix/lib/ld-linux-x86-64.so.2\" \"$@\"\n' \"$driver\" > $out/bin/$driver; chmod +x $out/bin/$driver; done"
