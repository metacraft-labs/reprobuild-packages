## Smoke test for the from-source ``knotifications`` recipe.
##
...
# ``knotifications`` at module init time.
import ./repro
...
suite "knotifications — from-source recipe smoke test":

  test "fetch spec carries the vendored URL verbatim":
    # M9.H registry round-trip — URL is recorded exactly as declared.
    let spec = registeredFetchSpec("knotifications")
    check spec.packageName == "knotifications"
  ...
  test "artifacts register a single library":
    ...
    let arts = registeredArtifacts("knotifications")
    check arts.len == 1
    check arts[0].packageName == "knotifications"
    check arts[0].artifactName == "libKF6Notifications"
    check arts[0].kind == dakLibrary

  test "versions block records the upstream tag + URL + repository":
    # M2 versions registry.
    let vs = registeredVersions("knotifications")
    check vs.len == 1
