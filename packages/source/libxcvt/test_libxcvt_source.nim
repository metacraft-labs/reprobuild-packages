...
suite "libxcvt source recipe":

  test "fetches the pinned X.Org release":
    let spec = registeredFetchSpec("libxcvt")
    check spec.packageName == "libxcvt"
  ...
  test "is a self-contained Meson leaf":
    check registeredBuildDeps("libxcvt").len == 0
    check registeredRuntimeDeps("libxcvt").len == 0
    let native = registeredNativeBuildDeps("libxcvt")
