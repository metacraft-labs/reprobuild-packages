## Smoke test for the from-source ``qt6ShaderTools`` recipe
## (M9.R.15n.2).
##
...
# fetch spec + artifacts under ``qt6ShaderTools`` at module
# init time.
import ./repro
...
suite "qt6ShaderTools — from-source recipe smoke test":

  test "fetch spec carries the upstream URL verbatim":
    let spec = registeredFetchSpec("qt6ShaderTools")
    check spec.packageName == "qt6ShaderTools"
  ...
  test "artifacts register the qsb executable + libQt6ShaderTools library":
    let arts = registeredArtifacts("qt6ShaderTools")
    check arts.len == 2
    check arts[0].packageName == "qt6ShaderTools"
  ...
  test "versions block records the upstream tag + URL + repository":
    let vs = registeredVersions("qt6ShaderTools")
    check vs.len == 1
