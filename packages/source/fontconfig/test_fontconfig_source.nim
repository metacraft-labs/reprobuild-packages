## Smoke test for the from-source ``fontconfig`` recipe.
##
...
# ``fontconfig`` at module init time.
import ./repro
...
suite "fontconfig — from-source recipe smoke test":

  test "fetch spec carries the vendored URL verbatim":
    # M9.H registry round-trip — URL is recorded exactly as declared.
    let spec = registeredFetchSpec("fontconfig")
    check spec.packageName == "fontconfig"
  ...
  test "artifacts register a single library":
    ...
    # (``lib/`` vs ``bin/``).
    let arts = registeredArtifacts("fontconfig")
    check arts.len == 1
    check arts[0].packageName == "fontconfig"
  ...
  test "versions block records the upstream tag + URL + repository":
    ...
    let vs = registeredVersions("fontconfig")
    check vs.len == 1
