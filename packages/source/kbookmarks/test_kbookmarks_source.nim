## Smoke test for the from-source ``kbookmarks`` recipe (M9.R.15h.8).
...
suite "kbookmarks — from-source recipe smoke test":

  test "fetch spec carries the upstream URL verbatim":
    let spec = registeredFetchSpec("kbookmarks")
  ...
  test "artifacts register libKF6Bookmarks":
    let arts = registeredArtifacts("kbookmarks")
    check arts.len == 1
