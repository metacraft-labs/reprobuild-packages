## Smoke test for the from-source ``kjobwidgets`` recipe (M9.R.15h.5).
...
suite "kjobwidgets — from-source recipe smoke test":

  test "fetch spec carries the upstream URL verbatim":
    let spec = registeredFetchSpec("kjobwidgets")
  ...
  test "artifacts register libKF6JobWidgets":
    let arts = registeredArtifacts("kjobwidgets")
    check arts.len == 1
